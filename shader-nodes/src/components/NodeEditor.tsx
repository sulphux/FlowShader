...
function EditorInner({ onChange }: Props) {
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    // UNIKALNA NAZWA DLA PARAMETRÓW
    let newLabel = def.label;
    if (typeId === 'param_float' || typeId === 'param_color') {
        let counter = 1;
        let uniqueNameFound = false;
        while (!uniqueNameFound) {
            const potentialName = counter === 1 ? def.label : `${def.label} ${counter}`;
            const exists = nodes.some(n => (n.data.label || n.data.definition.label) === potentialName);
            if (!exists) {
                newLabel = potentialName;
  ...
  const onAddNode = useCallback((typeId: string) => {
    ...
    // GENEROWANIE UNIKALNEJ NAZWY (dla Parametrów)
    let newLabel = def.label;
    if (typeId === 'param_float' || typeId === 'param_color') {
        let counter = 1;
        let uniqueNameFound = false;
  ...
  // --- BRAKUJĄCY ELEMENT Z POPRZEDNIEJ WERSJI ---
  ...
  return (
    <div ref={ref} style={{ width: '100%', height: '100%', background: '#111', position: 'relative' }} onMouseMove={onMouseMove}>
      ...
      <ReactFlow
        ...
        onEdgeContextMenu={onEdgeContextMenu} // Teraz już jest zdefiniowane!
      ...
      >
        ...
        {/* PORTAL SIDEBARA */}
        {document.getElementById('sidebar-root') && createPortal(
