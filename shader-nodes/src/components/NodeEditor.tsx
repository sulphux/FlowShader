...
import { compileGraphToGLSL, type GraphNode } from '../core/compiler'; // Potrzebne do podglądu kodu
...
function EditorInner({ onChange }: Props) {
  ...
  // ... (restoreGraph, handleSaveFile, handleLoadFileClick, handleFileChange, handleClear, deleteSelected bez zmian)
  ...
  const handleClear = useCallback(() => {
      if(window.confirm("Clear?")) { setNodes(initialNodesDefault); setEdges([]); localStorage.removeItem(STORAGE_KEY); }
  }, [setNodes, setEdges]);
  ...
  // NOWE: Funkcja generująca kod do podglądu
  ...
  // --- DRAG & DROP HANDLERS (Dla Sidebara) ---
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const newNode: Node = {
      ...
      type: isPreview ? 'previewNode' : 'shaderNode',
      position,
  ...
  // ... (reszta handlerów: onNodesChange, onConnect, onConnectStart/End, copy/paste, contextMenu, addNode) ...
  // Te funkcje zostają bez zmian (kopiuj z poprzedniego pliku), bo działają dobrze. 
  // Skracam kod tutaj dla czytelności, ale wklej pełne funkcje z poprzedniego kroku.
  const onNodesChange = useCallback((changes: any) => setNodes((nds) => applyNodeChanges(changes, nds)), [setNodes]);
  const onConnect: OnConnect = useCallback((params) => {
        const sourceNode = nodes.find(n => n.id === params.source); const targetNode = nodes.find(n => n.id === params.target);
        ...
        const sourceType = outputDef.type; const targetType = inputDef.type;
  ...
  const onAddNode = useCallback((typeId: string) => { if (!menu) return; const position = reactFlowInstance.screenToFlowPosition({ x: menu.x, y: menu.y }); const isGroup = typeId === 'special_group'; const isPreview = typeId === 'preview'; const newNodeId = `${typeId}_${Date.now()}`; const newNode: Node = { id: newNodeId, type: isPreview ? 'previewNode' : 'shaderNode', position, data: { definition: NODE_REGISTRY[typeId as keyof typeof NODE_REGISTRY] }, zIndex: isGroup ? -10 : 0, style: isGroup ? { width: 400, height: 300 } : undefined, }; setNodes((nds) => isGroup ? [newNode, ...nds] : nds.concat(newNode)); if (pendingConnection) { const { nodeId, handleId, handleType } = pendingConnection; const def = NODE_REGISTRY[typeId as keyof typeof NODE_REGISTRY]; let newEdge: Edge | null = null; if (handleType === 'source') { const input = def.inputs[0]; if (input) { newEdge = { id: `e_${nodeId}_${newNodeId}`, source: nodeId, sourceHandle: handleId, target: newNodeId, targetHandle: input.id, style: { stroke: '#fff', strokeWidth: 3 } }; } } else { const output = def.outputs[0]; if (output) { newEdge = { id: `e_${newNodeId}_${nodeId}`, source: newNodeId, sourceHandle: output.id, target: nodeId, targetHandle: handleId, style: { stroke: '#fff', strokeWidth: 3 } }; } } if (newEdge) { setEdges((eds) => addEdge(newEdge!, eds)); } setPendingConnection(null); } }, [menu, reactFlowInstance, setNodes, pendingConnection, setEdges]);
  ...
  return (
    <div ref={ref} style={{ width: '100%', height: '100%', background: '#111', position: 'relative' }} onMouseMove={onMouseMove}>
      <Toolbar 
        ...
        onClear={handleClear} 
        onShowCode={handleShowCode} // <--- NOWY PROP
      />
      ...
      <ReactFlow
        ...
        onDragOver={onDragOver} // <--- WĄŻNE DLA SIDEBAR
        onDrop={onDrop}         // <--- WAŻNE DLA SIDEBAR
      >
        ...
        {createPortal(
            <div onClick={deleteSelected} title="Delete Selected (Del)" style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 99999, width: 50, height: 50, borderRadius: '50%', background: '#ff007a', color: 'white', fontSize: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 5px 15px rgba(0,0,0,0.5)', transition: 'transform 0.1s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.1)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1.0)'}>🗑️</div>,
            document.body
        )}

        {menu && menu.visible && createPortal( <ContextMenu x={menu.x} y={menu.y} onClose={() => setMenu(null)} onAddNode={onAddNode} filterType={menuFilter} />, document.body )}

        {/* OKNO Z KODEM (MODAL) */}
        {showCode && createPortal(
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 100000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCode(false)}>
                <div style={{ background: '#1a1a1a', padding: '20px', borderRadius: '8px', border: '1px solid #444', width: '80%', height: '80%', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', color: '#fff', fontWeight: 'bold' }}>
                        <span>GENERATED GLSL</span>
                        <button onClick={() => setShowCode(false)} style={{ background: 'transparent', border: 'none', color: '#888', cursor: 'pointer', fontSize: '16px' }}>✕</button>
                    </div>
                    <textarea readOnly value={currentCode} style={{ flex: 1, background: '#111', color: '#81c784', border: 'none', fontFamily: 'monospace', padding: '10px', resize: 'none' }} />
                </div>
            </div>,
            document.body
        )}
