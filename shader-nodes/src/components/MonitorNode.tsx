...
export const MonitorNode = memo(({ id, selected }: NodeProps) => {
  ...
  // Hooki do śledzenia stanu grafu
  ...
  useEffect(() => {
      const updateShader = () => {
          ...
          try {
              ...
              // Ignorujemy błędy kompilacji w locie, żeby nie crashować UI
          }
      };

      const interval = setInterval(updateShader, 500); 
      updateShader(); // Init run
      return () => clearInterval(interval);
  }, [getNodes, getEdges, id]);

  // Pętla odczytu (Z OGRANICZNIKIEM FPS)
  useEffect(() => {
      ...
      const loop = () => {
          ...
          // THROTTLE: Aktualizuj tylko co 100ms (10 FPS)
          // To zapobiega zarzynaniu Reacta i przeglądarki
          const now = Date.now();
          ...
          if(rendererRef.current && sceneRef.current && meshRef.current && targetRef.current) {
              ...
              try {
                  ...
                  // Ustawiamy stan tylko jeśli komponent żyje
                  ...
                  // Error suppression during render
              }
          }
      };
    ...
    <>
      ...
      <div style={{
          ...
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              {(inputType === 'vec2' || inputType === 'vec3' || inputType === 'vec4') && (
                  <Row label="Y" value={values[1]} color="#69f0ae" />
              )}

              {(inputType === 'vec3' || inputType === 'vec4') && (
                  <Row label="Z" value={values[2]} color="#448aff" />
              )}

              {inputType === 'vec4' && (
                  <div style={{ borderTop: '1px solid #333', marginTop: '4px', paddingTop: '4px' }}>
                    <Row label="W" value={values[3]} color="#aaa" />
                  </div>
              )}
