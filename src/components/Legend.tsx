...
export default function Legend() {
  const [isOpen, setIsOpen] = useState(() => {
    const saved = localStorage.getItem('shader-editor-legend-open');
    return saved !== null ? JSON.parse(saved) : true;
  });

  useEffect(() => {
    localStorage.setItem('shader-editor-legend-open', JSON.stringify(isOpen));
  }, [isOpen]);

  return (
    <div style={{
      position: 'absolute',
      bottom: '20px', left: '20px', zIndex: 10,
      fontFamily: 'sans-serif', fontSize: '12px', color: '#eee',
      display: 'flex', alignItems: 'flex-end', flexDirection: 'column', gap: '10px'
    }}>
      <div 
        onClick={() => setIsOpen(!isOpen)}
        style={{
          background: '#333', padding: '8px 12px', borderRadius: '20px', cursor: 'pointer',
          border: '1px solid #555', userSelect: 'none', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', fontWeight: 'bold'
        }}
      >
